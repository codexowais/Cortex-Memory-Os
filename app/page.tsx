...
import { Memory, MemoryGraph as MemoryGraphType, ProactiveInsight } from "@/lib/memory/types";
            ...
            <Panel icon={<CalendarClock className="h-5 w-5 text-mint" />} title="Daily Cognitive Summary">
              ...
              <div className="mt-4 space-y-2">
                {(data?.summary.bullets ?? []).map((bullet) => (
                  <div key={bullet} className="rounded-lg border border-white/10 bg-white/[0.035] px-3 py-2 text-sm">
                    {bullet}
                  </div>
                ))}
              </div>
